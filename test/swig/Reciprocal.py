...
class ReciprocalTest(unittest.TestCase):
    ...
    def test_Reciprocal1D (self):
        npr = np.reciprocal(self.np_a)
        dcr = dc.reciprocal(self.dc_a)
        np.testing.assert_allclose(npr, np.array(dcr.data()).astype(np.float32),
                rtol=1e-3, atol=1e-3)

    def test_Reciprocal2D (self):
        np_a = np.reshape(self.np_a, (6,4))
        dc_a = dc.reshape(self.dc_a, (6,4))
        npr = np.reciprocal(np_a)
        dcr = dc.reciprocal(dc_a)
        np.testing.assert_allclose(npr.flatten(), np.array(dcr.data()).astype(np.float32),
                rtol=1e-3, atol=1e-3)

    def test_Reciprocal3D (self):
